...
This workspace now keeps only the latest experiments designed to support the
central claim: MLN rules can adapt to changing data streams while the foreground
response path remains realtime.
...
Batch MLN baselines must retrain or reselect rules on the rolling window after
each chunk. `Static_MaxEnt_Reference` is reported only as a non-adaptive
reference and is not treated as evidence of realtime adaptation.
...
`OnlineWeight_MLN` is faster but keeps a fixed rule structure and has much worse
log loss, so it is a useful lower-bound online baseline rather than an MLN
structure-adaptation competitor.
